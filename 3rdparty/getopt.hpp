...
#       ifdef _WIN32
        int argv;
        auto* list = CommandLineToArgvA(GetCommandLineA(), &argv);
        if (list) {
...
struct getoption : public std::map< std::string, std::string >
{
    using super = std::map< std::string, std::string >;

    getoption(int argc, const char** argv) : super() {
        // reconstruct vector
    ...
    getoption(const std::vector<std::string>& args) : super() {
        std::vector<const char*> argv;
        for (auto& it : args) {
            argv.push_back(it.c_str());
        }
        *this = getoption((int)argv.size(), argv.data());
    }
...
inline T getarg(const T& defaults, const char* argv) {
    static struct getoption map(getopt_utils::cmdline());
