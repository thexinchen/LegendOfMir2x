...
namespace
{
    ...
    bool textButton(const char *id, ImDrawList *list, const ImVec2 pos, const std::string &text, const bool selected = false)
    {
        ...
        const auto color = ImGui::IsItemHovered() ? IM_COL32(255, 0, 0, 255) : IM_COL32_WHITE;
        if(selected){
...
void ImRuntimeConfigBoard::draw() const
{
    ...
    if(beginWindow(m_boardSize)){
        ...
        for(int i = 0; i < to_d(mainLabels.size()); ++i){
            ImGui::PushID(i);
            if(textButton("##runtime-main-page", list, {menuMin.x + 18, menuMin.y + 65 + i * 45.0f}, mainLabels[i], m_mainPage == i)){
                m_mainPage = i;
            }
            ImGui::PopID();
        }
        ...
        if(m_mainPage == 0){
            ...
            if(m_systemTab == 0){
                ...
                ImGui::SetCursorScreenPos({pagePos.x + 50, pagePos.y + 35});
                ImGui::SetNextItemWidth(100);
                const auto windowLabel = str_printf("%d×%d", windowSize.first, windowSize.second);
                if(ImGui::BeginCombo("##runtime-resolution", windowLabel.c_str())){
                ...
                ImGui::SetCursorScreenPos({pagePos.x + 50, pagePos.y + 65});
                ImGui::SetNextItemWidth(140);
                const std::array<const char *, 3> imeLabels {{"禁用", "使用内置输入法", "使用系统输入法"}};
                if(ImGui::BeginCombo("##runtime-ime", imeLabels.at(ime))){
                ...
                const auto slider = [&](const char *id, const char *label, const float y, const bool active, float value, const int config)
                {
                    ...
                    ImGui::SetCursorScreenPos({pagePos.x + 65, pagePos.y + 35 + y});
