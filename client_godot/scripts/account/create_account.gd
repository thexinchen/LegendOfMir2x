...
@onready var password_check: Label = %PasswordCheck
...
func _submit() -> void:
	...
	if not Validation.is_email(account_input.text):
		...
		_clear_all()
	elif not Validation.is_password(password_input.text):
		_show_status("无效密码", 2000.0)
		password_input.clear()
		confirm_input.clear()
	elif password_input.text != confirm_input.text:
		_show_status("两次密码输入不一致", 2000.0)
		password_input.clear()
		confirm_input.clear()
	else:
		_show_status("提交中")
...
func _on_server_message(head_code: int, payload: PackedByteArray) -> void:
	if head_code == NetworkClient.SM_CREATEACCOUNTOK:
		...
		_clear_all()
		_show_status(messages.get(error_code, "注册失败"), 2000.0)
...
func _on_status_active_changed(active: bool) -> void:
	...
	password_input.editable = not active
...
func _update_checks() -> void:
	...
	_set_check(password_check, password_input.text, Validation.is_password(password_input.text), active)
	_set_check(confirm_check, confirm_input.text, Validation.is_password(confirm_input.text) and confirm_input.text == password_input.text, active)
