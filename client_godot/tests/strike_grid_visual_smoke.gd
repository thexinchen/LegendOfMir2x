...
func _ready() -> void:
	...
	GameState.strike_grids = {"371,132": Time.get_ticks_msec() - 500}
	...
	if output_path.is_empty():
		output_path = "/tmp/mir2x-strike-grid.png"
	var error := get_viewport().get_texture().get_image().save_png(output_path)
	if error != OK:
		_fail("unable to save screenshot: %s" % error)
		return
	print("STRIKE GRID VISUAL PASS: %s" % output_path)
	get_tree().quit()


func _fail(message: String) -> void:
	push_error("STRIKE_GRID_VISUAL %s" % message)
	get_tree().quit(1)
