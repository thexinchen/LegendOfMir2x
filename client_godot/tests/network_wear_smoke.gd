...
enum Stage { WAIT_READY, WAIT_EQUIPPED, WAIT_RETURNED, WAIT_PLACED }
...
func _process(_delta: float) -> void:
	...
	match _stage:
		Stage.WAIT_READY:
			if not _inventory_received or not GameState.grabbed_item.is_empty() or not _wear_item().is_empty():
				return
			var initial_item := _inventory_item()
			if initial_item.is_empty():
				return
			_initial_seq_id = int(initial_item.get("seqID", 0))
			inventory_panel.show()
			if not _emit_inventory_item_click(inventory_panel, "%d:%d" % [ITEM_ID, _initial_seq_id], MOUSE_BUTTON_RIGHT):
				return
			_stage = Stage.WAIT_EQUIPPED
		Stage.WAIT_EQUIPPED:
			if int(_wear_item().get("itemID", 0)) != ITEM_ID or not _inventory_item().is_empty():
				return
			...
			if weapon_slot == null:
				return
			if OS.has_environment("MIR2X_NETWORK_WEAR_SCREENSHOT"):
				await RenderingServer.frame_post_draw
				var error := get_viewport().get_texture().get_image().save_png(OS.get_environment("MIR2X_NETWORK_WEAR_SCREENSHOT"))
				if error != OK:
					_fail("failed to save equipped weapon screenshot: %s" % error, 8)
					return
			weapon_slot.pressed.emit()
			_stage = Stage.WAIT_RETURNED
		Stage.WAIT_RETURNED:
			if not _wear_item().is_empty() or int(GameState.grabbed_item.get("itemID", 0)) != ITEM_ID:
				return
			...
			inventory_panel.get_node("ItemGrid").gui_input.emit(event)
		...
		Stage.WAIT_PLACED:
			...
			if restored.is_empty():
				return
			if not _wear_item().is_empty() or int(restored.get("count", 0)) != 1:
				_fail("returned weapon state is inconsistent: inventory=%s wear=%s" % [restored, _wear_item()], 9)
				return
			print("NETWORK WEAR PASS: item=%d:%d inventory->weapon->cursor->inventory returned_seq=%d" % [ITEM_ID, _initial_seq_id, int(restored.get("seqID", 0))])
			_finished = true
			NetworkClient.disconnect_from_server()
			get_tree().quit()


func _emit_inventory_item_click(panel: Control, key: String, button: MouseButton) -> bool:
...
func _on_timeout() -> void:
	_fail("timed out stage=%d inventory=%s wear=%s grabbed=%s" % [_stage, _inventory_item(), _wear_item(), GameState.grabbed_item], 6)
...
func _fail(message: String, code: int) -> void:
	if _finished:
		return
	_finished = true
	push_error("NETWORK_WEAR_SMOKE %s" % message)
	NetworkClient.disconnect_from_server()
	get_tree().quit(code)
